...
Локальный выход удаляет сохранённые credentials, но не заявляет отзыв сессии на
...
Перед удалением остановите auth broker. После logout перезапустите MCP-клиенты.

## Общая авторизация для двух MCP

Каждый MCP может подключаться отдельно. Banking MCP по умолчанию продолжает
работать напрямую с session-файлом. Для одной mobile session на Banking и
Hotels запустите локальный broker после входа:
