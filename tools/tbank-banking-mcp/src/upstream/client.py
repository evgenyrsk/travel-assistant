...
MAX_HOTEL_VOUCHER_BYTES = 5 * 1024 * 1024
...
class MobileSession:
    ...
    def _call_read(self, template_key: str, *, overrides: dict | None = None,
        ...
        if method == "POST":
            ...
            r = self._http.get(url, params=params, headers=headers, timeout=30, stream=stream_response)
        if return_response:
...
# ---- messenger / support chat (tm.t-bank-app.ru) — Bearer+cookie, no sig ----
    ...
    def hotel_voucher(self, booking_id: str) -> tuple[bytes, str]:
        """Voucher PDF for a confirmed own booking.

        The response object stays inside the local auth broker boundary so the
        document bytes can be size/type checked and written to owner-only local
        storage without entering MCP JSON or the model context.
        """
        response = self._call_read(
            "hotel_voucher",
            path_override=f"/api/v1/hotels/bookings/voucher/{booking_id}",
            return_response=True,
        ...
        if declared_length is not None:
            try:
                ...
                parsed_length = None
            if parsed_length is not None and parsed_length > MAX_HOTEL_VOUCHER_BYTES:
                response.close()
        ...
        try:
            for chunk in response.iter_content(chunk_size=64 * 1024):
                ...
                if len(content) > MAX_HOTEL_VOUCHER_BYTES:
                    raise ValueError("voucher PDF exceeds the 5 MiB safe size limit")
        finally:
            response.close()
        return bytes(content), str(response.headers.get("content-type") or "")

    def hotel_customer_data(self) -> dict:
        """Current Hotels customer via the live-verified Bearer-only profile."""
        data = self._call_read("hotel_customer_data")
        return data if isinstance(data, dict) else {}

    def hotel_bookings_list(
        self,
        *,
        active: bool,
        cancelled: bool,
        ...
        """Hotels booking lists via the live-verified Bearer-only profile."""
        data = self._call_read("hotel_bookings_list", body={
            "isActiveRequired": active,
            "isCancelledRequired": cancelled,
            "isCompletedRequired": completed,
        })
        return data if isinstance(data, dict) else {}
