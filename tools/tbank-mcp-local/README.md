...
```bash
npx -y tbank-mcp-local@0.15.0 connect cursor
```
...
```bash
npx -y tbank-mcp-local@0.15.0 connect codex
```
...
```bash
npx -y tbank-mcp-local@0.15.0 connect opencode
```
...
```bash
npx -y tbank-mcp-local@0.15.0 connect cursor --profile hotels
```
...
Текущий совместимый публичный набор: `tbank-hotels-mcp@0.30.0`,
...
`UNLICENSED`; набор `0.30.0/0.17.0/0.16.0` прошёл review, bounded smoke,
registry upload и fresh-install initialize. До полного stable release остаются выбор публичной лицензии,
