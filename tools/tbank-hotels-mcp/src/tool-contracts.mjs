...
export const tools = [
  ...
  },
  {
    ...
    description: "Создаёт безопасный локальный preview выбранного отеля, тарифа и состава гостей без ФИО, email и телефона. Требует уже завершённый tbank_hotels_get_selected_stay_rates и отдельный tbank_hotels_select_stay_rate; не вызывайте preview одновременно с rates или до выбора rateOptionId. Не создаёт booking draft, не вызывает Hotels API и не запрашивает PII. В обычном пользовательском ответе не раскрывайте внутренние имена заголовков или конфигурационные blockers; достаточно сообщить, что реальная бронь не создавалась.",
  ...
  },
  {
    ...
    description: "Создаёт безопасный переход из выбранного hotel/rate preview на публичную страницу конкретного выбранного отеля. Для обычного сценария одной комнаты без детей URL сохраняет выбранный отель, даты и число взрослых через подтверждённые public-параметры dateFrom/dateTo/guests; сложный состав гостей не угадывается. Не принимает и не передаёт ФИО, контакты, bookHash, токены, данные карты, OTP или 3-D Secure; не создаёт бронь и не запускает оплату. Точный тариф не переносится и не резервируется: пользователь выбирает актуальный тариф во внешнем интерфейсе.",
    inputSchema: objectSchema({ journeyId: identifierSchema("Непрозрачный journeyId с выбранным rateOptionId.") }, ["journeyId"]),
  },
  {
