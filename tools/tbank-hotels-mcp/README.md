...
Каноническая форма выше остаётся рекомендуемой. Для устойчивости разных
LLM-клиентов journey boundary также принимает ограниченные алиасы `location`,
`guests`, `limit`, `adultsCount` и `childrenAge`, нормализует их локально до
provider-вызова и отклоняет одновременную передачу алиаса и канонического поля.
Неизвестные поля по-прежнему отклоняются.
...
`truncated`. Поля `coverageLevel` (`complete`, `substantial`, `partial`) и
`coverageRatio` показывают полноту относительно reported count текущего
provider search lifecycle, а не всего каталога локации. Неполная выборка с
ratio от 80% считается `substantial`; ниже 80% — `partial`.

Если неполная выборка остановилась по time budget или bounded loading polls,
`continuationAvailable=true`. При действительно низком покрытии
`continuationRecommended=true`, и агент один раз вызывает
`tbank_hotels_continue_stay_search` перед сравнением лучших вариантов. Tool
продолжает тот же journey с текущего offset, сохраняет прежние `optionId` и
общий лимит initial+continuation в 20 search-запросов. Усечённый результат не
сохраняется как финальный global cache; завершённый результат после
continuation может использоваться обычным коротким cache. Повторное
продолжение без явного запроса пользователя запрещено не только guidance:
после первого continuation MCP механически возвращает
`continuationRecommended=false`, сохраняя `continuationAvailable=true` только
для явного запроса на более полный поиск. Если выбранный отель исчез из
обновлённой provider-выборки, selection и загруженные rates сбрасываются, а
ответ требует нового выбора. Поэтому механизм не превращается в скрытый
нагрузочный цикл. `maxOptions` влияет только
на число вариантов непосредственно в ответе `plan_stay`, но не обрезает
