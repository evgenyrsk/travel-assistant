...
| Level | Описание | User value | Required capabilities | Required data | Complexity | MVP recommendation | Risks | Related scenarios/use cases |
|---|---|---|---|---|---|---|---|---|
| Level 1 — Combined intent recognition | Ассистент понимает, что пользователь хочет связку отель + перелет или открытую поездку. | Пользователь может начать с естественной формулировки поездки целиком. | Intent classification, extraction of shared constraints, clarification trigger. | intent type, origin/destination if present, dates/duration, travelers, budget, preferences. | Low/Medium. | **In MVP.** Следует из FR-001 и S-03. | Ошибочно принять combined request за hotel-only или flight-only. | S-03, UC-03, UC-11 |
| Level 2 — Same-dialog hotel and flight assistance | Ассистент помогает отдельно подобрать перелет и отдельно отель в одной search session, сохраняя общий контекст. | Пользователь не повторяет параметры и видит связанный диалог. | Shared search session, parameter reuse, separate hotel/flight search, basic context update. | shared dates, origin, destination, travelers, hotel preferences, flight preferences, budget. | Medium. | **In MVP.** Совместим с Stage 1: hotel и flight search входят в MVP, а combined intent должен сохранять общий контекст. | Пользователь может ожидать package ranking, которого еще нет. Нужно явно обозначать пределы. | S-01, S-02, S-03, UC-01, UC-02, UC-03 |
| Level 3 — Coordinated combined search | Ассистент согласовывает параметры между отелем и перелетом: даты, бюджет, город, длительность, состав путешественников. | Пользователь получает более жизнеспособные варианты поездки, а не два независимых списка. | Constraint coordination, budget split assumptions, consistency checks, partial combined fallback. | total budget, flight budget part/open, hotel budget part/open, shared dates, destination, provider facts for both offer types. | Medium/High. | **Open for Stage 3.** Может войти в MVP, если Stage 3 подтвердит минимальный уровень coordination без full package ranking. | Неявное расширение MVP; budget split может стать недостоверным без данных; provider limitations. | S-03, S-09, S-10, UC-03, UC-10, UC-12 |
| Level 4 — Full combined package ranking | Ассистент ранжирует готовые пакеты перелет + отель как единую сущность с общей ценой и trade-offs. | Пользователь видит готовые package recommendations. | Package construction, combined price, compatibility validation, package-level ranking, richer freshness and availability handling. | confirmed flight offers, confirmed hotel offers, compatibility rules, combined price, taxes/fees if available, freshness for both sides. | High. | **Post-MVP or Open.** Stage 1 не требует full package ranking как обязательную MVP-сущность. | Сильный scope creep; риск ложной уверенности; сложность объяснения и freshness. | S-03, UC-03, UC-05, UC-06 |
