...
| ID | Rule | Rationale | Examples | Related use cases | MVP status |
|---|---|---|---|---|---|
...
| ABR-017 | Если API-контракт еще не предоставлен, ассистентские сценарии должны фиксировать это как open input для technical stages. | Это не меняет MVP scope: real API integration остается частью MVP. | Contract placeholders допустимы только как временная поддержка разработки. | UC-15 | Open input |
