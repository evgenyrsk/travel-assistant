...
- Stage 14.0 - подтвердил рабочий `demo-ready MVP`; Stage 14.1a и Stage 14.1b
  ...
  optional image contract. Responsive redesign Stage 14.1c и реальные
  изображения закрыты Stage 14.6 после подтверждения provider template.
  Stage 14.2 закрыл критическую ошибку определения года, добавил безопасные
  правила интерпретации дат и закрепил один номер как значение по умолчанию для
  assistant flow. Stage 14.3 добавил подтверждённое требование включённого
  завтрака, Stage 14.4 разделил пользовательские сообщения для временного
  сбоя и неоднозначного разбора без раскрытия внутренних причин, Stage 14.5
  закрыл пропуск точной пятизвёздочной формулировки, а Stage 14.6 разрешил
  provider image template без proxy или N+1.
- Stage 14.7 - завершён для явно названного отеля: длительность проживания
  сохраняется между уточнениями, а наличие и цена проверяются через внутреннюю
  exact-hotel ветку без раскрытия provider identity. Если валидный LLM
...
- Stage 16 - активирован для semantic-анализа типа размещения. Единственный
  активный concept — `GLAMPING`; REAL vision остаётся заблокированным до
  подтверждения прав на передачу provider content и quality gate.
...
image из уже полученного search response. Stage 14.1c объясняет
действующее ранжирование один раз над списком и не повторяет одинаковый
`matchSummary` в каждой карточке. Stage 14.6 подтвердил provider templates и
реальную загрузку пяти изображений в demo shell.

Stage 14.2 использует IANA timezone клиентского устройства только как
необязательную подсказку. Текущая локальная дата вычисляется по backend `Clock`:
клиентский timestamp не принимается как источник истины. При отсутствующей или
некорректной timezone относительные и не содержащие год даты требуют
уточнения, а дата заезда в прошлом блокируется до confirmation и provider
search. Если пользователь не назвал количество номеров, assistant flow
использует один номер. Текущий provider contract поддерживает только один
номер на один поиск: явно запрошенные два и более номера блокируются до
confirmation без provider call и без молчаливого объединения гостей. Для
второго номера пользователь может выполнить отдельный поиск. Обычный
confirmation и demo UI не показывают этот внутренний параметр.

Stage 14.3 подтвердил по обезличенному provider catalog фильтр
`meal_types=["breakfast"]` и добавил пятое необязательное preference —
включённый завтрак. В публичном offer факт остаётся nullable: `breakfast` и
`nomeal` преобразуются явно, остальные meal types не угадываются.

Stage 14.7 различает направление и конкретный отель. Location ID используется
обычным поиском, а строковый hotel reference — только внутренними details/rates
вызовами после confirmation. Для exact-hotel результата выбирается один
...
- итеративное уточнение в чате максимальной общей стоимости, звезд,
  ...
  `9`, обязательной бесплатной отмены и включённого завтрака;
- точная одиночная категория звёзд из явной формулировки дополнительно
  проверяется application-policy и не зависит только от полноты LLM patch;
...
- повторное подтверждение полного набора критериев перед каждым новым provider
  search;
- ровно один номер на один поиск: это значение используется по умолчанию, а
  запрос нескольких номеров требует отдельных поисков;
- интерпретацию относительных дат только при наличии корректной client timezone
  и backend-owned текущего времени;
...
- on-demand детали одного явно выбранного offer без автоматической загрузки
  остальных карточек;
- проверку availability и общей цены явно названного единственного hotel
  candidate после подтверждения, без преобразования hotel ID в destination ID;
...
Публичный rates/rooms flow, deeplink, shortlist и comparison остаются
отдельными направлениями и не входят в MVP. Внутренний v3 rates request
используется только для availability/price явно названного отеля. Flight search может быть активирован только новым
...
5. После подтверждения application/provider layer выполняет один ограниченный
   ...
   Если запрос однозначно указывает конкретный отель, backend вместо
   destination search выполняет ограниченную ветку details + rates.
...
- Порядок предложения ослабления: минимальный гостевой рейтинг, звёзды,
  бесплатная отмена, включённый завтрак, максимальная общая стоимость. Бюджет
  предлагается снять последним.
