...
Верхнеуровневый продуктовый roadmap находится в `docs/ROADMAP.md`, поэтапный roadmap — в `docs/roadmap/roadmap.md`. Этот файл является детальным roadmap разработки и не должен использоваться для преждевременного начала будущих этапов.
...
**Результат:** `FlightSearchClient` или аналогичный порт, mock-провайдер и доменные модели перелетов.
...
**Результат:** `HotelSearchClient` или аналогичный порт, mock-провайдер и доменные модели проживания.
...
**Результат:** базовый сервис ранжирования, который сортирует mock-предложения и формирует причины рекомендации.
...
- mock-провайдеры работают без внешних учетных данных.
