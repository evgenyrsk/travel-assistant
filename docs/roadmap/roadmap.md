...
| Пункт | Статус |
|---|---|
| Текущий этап | Нет активного implementation stage; Stage 16.8a завершён, Stage 16.8b не начат |
| Последний завершенный этап | Stage 16.8a — backend semantic runtime safety |
| Следующий планируемый шаг | Stage 16.8b — отдельным этапом исправить async UX и добавить прозрачность активных runtime modes |
...
| Stage 16 | Stage 16.0–16.8a завершены в разрешённом scope; Stage 16.8b следующий | Provider-neutral async two-pass semantic-анализ `GLAMPING`, public polling, observability и evaluation harness прошли regression gates; смешанный REAL Hotels + FAKE semantic runtime теперь завершается fail-closed до provider/analyzer calls. |
...
**Статус:** Stage 16.0–16.8a завершены в разрешённом scope. Смешанный REAL
Hotels + FAKE semantic runtime закрыт fail-closed; Stage 16.8b остаётся следующим
разрешённым отдельным шагом и ещё не начат. REAL rollout не активирован.
...
`docs/reviews/stage-16-8-semantic-runtime-safety-plan.md`, а результат Stage 16.8a —
в `docs/reviews/stage-16-8a-backend-semantic-runtime-safety.md`. Stage 16.8 не меняет
